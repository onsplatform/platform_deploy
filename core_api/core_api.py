...
class CoreApi:
    ...
    def upload_maps(self, solution,app):
        path = getcwd() + '/Mapa/'
        map_names = [f for f in listdir(path) if isfile(join(path, f)) and (f.endswith('.yaml') or f.endswith('.yml'))]
    ...
    def upload_operations(self,solution,app):
        ...
        metadata_names = [f for f in listdir(path) if isfile(join(path, f)) and (f.endswith('.yaml') or f.endswith('.yml'))]
