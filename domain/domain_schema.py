...
class DomainSchema:
    ...
    def update_solution(self, solution):
        response = requests.put(url=self.url + 'solution/{id}/'.format(id=solution['id']), data=json.dumps(solution),
    ...
    def create_app(self, app, solution, tag):
        new_app = self._get_app_from_config(app, solution, tag)
    ...
    def update_app(self, app, solution, tag):
        new_app = self._get_app_from_config(app, solution, tag)
        new_app['id'] = app['id']
        response = requests.put(url=self.url + 'app/{id}/'.format(id=app['id']), data=json.dumps(new_app),
                                headers=self.headers)
        self.create_app_version(app, tag)
        return response.json()
    ...
    def _get_app_from_config(self, app, solution, tag):
        return {
            'name': app['name'],
